	...
	return {
		...
		submit: async (input): Promise<FormSubmissionState<Result>> => {
			...
			if (!validation.valid || validation.data === undefined) {
				return update({ status: 'error', errors: validation.errors, message: options.errorMessage ?? '' })
			}
			...
			try {
				...
				const message = error instanceof Error ? error.message : options.errorMessage ?? 'Submission failed'
